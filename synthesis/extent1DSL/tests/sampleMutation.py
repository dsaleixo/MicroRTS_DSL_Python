...
class SampleMutation():
    ...
    @staticmethod
    def test2():
        map = "maps/24x24/basesWorkers24x24A.xml"
        max_tick = 2000
        sp = SelfPlayDefault(1,map,max_tick,Factory_E1())
        prog_current = sp.getBest()
        score_current = sp.evaluate(prog_current)
        ...
        while True:
            ...
            for _ in range(30):
                ...
                #print(Control.save(no))
               # print(mutation.translate())
                score = sp.evaluate(mutation)
                print(str(score)+" "+str(score_current))
                if score>score_current:
                    ...
                    sp.update(mutation)
                    score_current = sp.evaluate(mutation)
